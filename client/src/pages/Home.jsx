...
const Home = () => {
  ...
  // Setup displayCoin whenever allCoin, favorites, searchQuery, or activeTab change
  useEffect(() => {
  ...
  }, [allCoin, favorites, user, activeTab, searchQuery]);
  ...
  // Fixed favorite handler - only prevent default, don't stop propagation
  const handleFavClick = async (e, coin) => {
    e.preventDefault(); // Only prevent default to avoid link navigation
    // Removed e.stopPropagation() as it was interfering with parent Link
          ...
          {displayCoin.slice(0, 20).map((coin) => {
            ...
            return (
              <Link 
                key={coin.id} 
                to={`/coin/${coin.id}`}
                className={`group relative block ${darkMode ? 'bg-gradient-to-br from-[#1e1e1e] to-[#292929]' : 'bg-gradient-to-br from-white to-gray-100'} rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border ${borderColor} hover:-translate-y-1`}
              >
                <div className="p-6">
                  {/* Favorite Button */}
                  {user && (
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-80 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="text-center p-4">
                        ...
                        <Link
                          ...
                          onClick={(e) => e.stopPropagation()} // Use stopPropagation here instead
                        >
                          Sign In
                        </Link>
                      </div>
